...
namespace Kakeibo
{
	class KBDatabase : IDisposable
	{
		...
		public int getTotalPayment(DateTime stDate, DateTime edDate)
		{
			int ret = 0;
			ret = getMoney("ie_type = 0", stDate, edDate);
		...
		public int getTotalCashPayment(DateTime stDate, DateTime edDate)
		{
			object obj;
			int ret = 0;
			ret = getMoney("ie_type = 0 and id_account = (select id from T_BALANCE where (type = 0 OR type = 2))", stDate, edDate);
		...
		public KBDat getKBDat()
		{
			...
			szCmd = new SQLiteCommand(szQuery, hConn);
			reader = szCmd.ExecuteReader();
			szCmd = new SQLiteCommand(szQuery, hConn);
			while (reader.Read())
			{
			}
